...
async function fetchJsonCached(url) {
  ...
  function buildChart(canvas, elevPoints, movingMarker, surfaceSummaryEl, route, unknownAsTrail) {
    ...
    // Tegn hvert stykke mellom to punkter med underlaget som er
    // registrert på det siste punktet. Begge endepunktene tas med,
    // slik at det ikke blir hvite sprekker ved underlagsskifter.
    for (let i = 1; i < elevations.length; i++) {
      const cat = pointsByCategory[cats[i]] ? cats[i] : "unknown";
      const targetPoints = pointsByCategory[cat];

      targetPoints[i - 1].y = elevations[i - 1];
      targetPoints[i].y = elevations[i];
  ...
  async function initRouteSection(section) {
    try {
      ...
      const routeMarkersUrl = (
        section.dataset.routeMarkersUrl || ""
      ).trim();

      const unknownAsTrail =
        String(
          section.dataset.unknownAsTrail ||
            ""
        ).trim() === "1";
      ...
      const mapDiv =
        section.querySelector(".route-map");
      ...
      if (typeof L === "undefined") {
        console.error(
          "[route_map] Leaflet (L) is undefined"
        );
        return;
      }

      if (section.__routeMapInitialized) {
        return;
      }
      ...
      const routesJson =
        await fetchJsonCached(routesUrl);
      ...
      try {
        const elevJson =
          await fetchJsonCached(elevUrl);

        const pts = Array.isArray(
          elevJson.points
        )
          ? elevJson.points
          : elevJson;
