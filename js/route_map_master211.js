...
(function () {
  ...
  console.log("[route_map] master211 loaded");
  ...
  function showPoiCard(popupContainer, poi, resetFn) {
    ...
    // Vi bytter innhold – men resetFn tar deg tilbake, og posisjonsboksen blir bygget på nytt av resetFn.
    popupContainer.innerHTML = `
      <div class="poi-card">
  ...
  function addMarkerFromDb(mapOrLayer, poi, popupContainer, resetFn) {
    ...
    // Hover viser POI, men dette har NULL å si for posisjon-beregningen.
  ...
  function renderPositionResult(popupContainer, routeIndex, lat, lon) {
    ...
    // Hvis POI-card er aktiv: ikke prøv å "lime inn" posisjon der – vi oppdaterer bare når vi er på stats-visning.
    // Løsning: hvis .stats-box finnes, oppdater pos-boksen. Hvis ikke, gjør ingenting.
  ...
  // IMPORTANT: takes getRouteIndex() so it works even if chart loads later.
  // Optional: revealPoisNear(latlng,radius) to "boost" POIs near chosen point
  // ======================
  function addPositionControl(map, popupContainer, getRouteIndex, revealPoisNear, resetPopup) {
    if (!map) return;
      ...
      L.DomEvent.on(btnMe, "click", (e) => {
        ...
        navigator.geolocation.getCurrentPosition(
          (pos) => {
            ...
            // tilbake til nøkkelinformasjon hvis du sto i en POI
  ...
  async function initRouteSection(section) {
    try {
      ...
      // Boost: show all POIs near chosen point (regardless of anchor policy)
      ...
      if (enablePosition) {
        addPositionControl(
          ...
          revealPoisNear,
          () => {
            // tilbake til nøkkelinformasjon og oppdater posisjon hvis vi har en sist kjent pos
            resetPopup();
          }
        );
      }
  ...
  // - Retry for a while (DOM may appear after script loads)
  // - MutationObserver for late injected blocks
  // ======================
  ...
  function startRobustInit() {
    let tries = 0;
    const maxTries = 60; // ~15s (60 * 250ms)
    ...
    // Start now + also after DOMContentLoaded
    tick();
    ...
    // Observer for late DOM injection
    try {
      const obs = new MutationObserver(() => initAllOnce());
      obs.observe(document.documentElement, { childList: true, subtree: true });
      // stop after a while to avoid endless work
      setTimeout(() => {
