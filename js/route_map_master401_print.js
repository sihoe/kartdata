...
(function () {
  ...
  console.log("[route_map] master401 print loaded");
  ...
  const infoTexts = {
    no: {
      ...
      printTitle: "Skriv ut turen",
      printButton: "Skriv ut / lagre som PDF",
      closeButton: "Lukk",
      printLoading: "Høydeprofilen er ikke ferdig lastet. Prøv igjen om et øyeblikk.",
      detailTitle: "Detaljkart",
      detailIntro: "Utsnittene overlapper. Følg rutelinjen og pilene i sykkelretningen.",
      detailPart: "Del",
      detailFooter: "Skann QR-koden på side 1 for interaktivt rutekart.",
      detailScale: "Målestokk 1:50 000",
      qrInstruction: "Skann QR-kode for interaktivt rutekart",
      qrError: "QR-kode kunne ikke lastes",
      startMarker: "S",
      finishMarker: "M",
    },
    ...
    en: {
      ...
      printTitle: "Print route",
      printButton: "Print / save as PDF",
      closeButton: "Close",
      printLoading: "The elevation profile is still loading. Please try again in a moment.",
      detailTitle: "Detailed maps",
      detailIntro: "The sections overlap. Follow the route line and arrows in the direction of travel.",
      detailPart: "Part",
      detailFooter: "Scan the QR code on page 1 for the interactive route map.",
      detailScale: "Scale 1:50,000",
      qrInstruction: "Scan QR code for interactive route map",
      qrError: "The QR code could not be loaded",
      startMarker: "S",
      finishMarker: "F",
    },
    ...
    de: {
      ...
      printTitle: "Route drucken",
      printButton: "Drucken / als PDF speichern",
      closeButton: "Schließen",
      printLoading: "Das Höhenprofil wird noch geladen. Bitte versuchen Sie es gleich noch einmal.",
      detailTitle: "Detailkarten",
      detailIntro: "Die Ausschnitte überlappen sich. Folgen Sie der Routenlinie und den Pfeilen in Fahrtrichtung.",
      detailPart: "Teil",
      detailFooter: "Scannen Sie den QR-Code auf Seite 1 für die interaktive Routenkarte.",
      detailScale: "Maßstab 1:50.000",
      qrInstruction: "QR-Code für interaktive Routenkarte scannen",
      qrError: "Der QR-Code konnte nicht geladen werden",
      startMarker: "S",
      finishMarker: "Z",
    },
  };
  ...
  function ensurePrintStyles() {
    ...
    style.textContent = `
      ...
      .svingom-print-footer{display:grid;grid-template-columns:18mm minmax(0,1fr) 38mm;gap:5mm;align-items:end;border-top:1px solid #d7cec3;padding-top:2mm;min-height:0}
      .svingom-print-info-block{align-self:end;display:flex;flex-direction:column;justify-content:space-between;height:18mm;min-width:0}
      .svingom-print-stats{display:flex;flex-wrap:wrap;justify-content:center;text-align:center;gap:2mm 6mm;font-size:9px;line-height:1.2}
      .svingom-print-stats strong{font-size:11px}
      .svingom-print-qr{display:flex;align-items:flex-end;align-self:end;gap:2mm}
      ...
      .svingom-print-route-note{font-size:8px;line-height:1.1;color:#422426;white-space:nowrap}
      ...
      .svingom-direction-arrow{width:0;height:0;border-left:6px solid transparent;border-right:6px solid transparent;border-bottom:14px solid #37394E;filter:drop-shadow(0 0 1px #fff) drop-shadow(0 0 1px #fff);transform-origin:50% 50%}
      .svingom-north-control{display:flex;flex-direction:column;align-items:center;justify-content:center;width:30px;height:40px;background:rgba(255,255,255,.92);border:1px solid rgba(66,36,38,.35);border-radius:3px;color:#422426;font:800 12px/1 sans-serif;box-shadow:0 1px 4px rgba(0,0,0,.18)}
      .svingom-north-control .arrow{font-size:22px;line-height:18px;margin-top:-1px}
      .svingom-fixed-scale{padding:3px 6px;background:rgba(255,255,255,.92);border:1px solid rgba(55,57,78,.45);border-radius:2px;color:#37394E;font:700 10px/1.2 sans-serif;box-shadow:0 1px 3px rgba(0,0,0,.14)}
  ...
  function printZoomForScale(lat, scale = 50000) {
    const metersPerCssPixel = scale * 0.0254 / 96;
    const latitudeFactor = Math.max(
      0.15,
      Math.cos((Number(lat) || 60) * Math.PI / 180)
    );
    return Math.log2(
      (156543.03392 * latitudeFactor) / metersPerCssPixel
    );
  }

  function addNorthControl(map) {
    const control = L.control({ position: "topright" });
    control.onAdd = function () {
      const el = L.DomUtil.create("div", "svingom-north-control");
      el.innerHTML = '<span>N</span><span class="arrow">↑</span>';
      return el;
    };
    control.addTo(map);
  }

  function addFixedScaleControl(map, label) {
    const control = L.control({ position: "bottomleft" });
    control.onAdd = function () {
      const el = L.DomUtil.create("div", "svingom-fixed-scale");
  ...
  function detailSegments(routeIndex) {
    const distances = routeIndex.distances || [];
    const lats = routeIndex.lats || [];
    const lons = routeIndex.lons || [];
    const last = Math.min(distances.length, lats.length, lons.length) - 1;
    if (last < 0) return [];

    const maxWidthM = 8500;
    const maxHeightM = 4800;
    const overlapKm = 0.5;
    const segments = [];
    let start = 0;

    while (start < last) {
      let minLat = Number(lats[start]);
      let maxLat = minLat;
      let minLon = Number(lons[start]);
      let maxLon = minLon;
      let end = Math.min(last, start + 1);
      ...
      for (let i = start + 1; i <= last; i++) {
        ...
        const nextMinLat = Math.min(minLat, lat);
        const nextMaxLat = Math.max(maxLat, lat);
        const nextMinLon = Math.min(minLon, lon);
        const nextMaxLon = Math.max(maxLon, lon);
        const midLat = (nextMinLat + nextMaxLat) / 2;
        const heightM = (nextMaxLat - nextMinLat) * 111320;
        const widthM =
          (nextMaxLon - nextMinLon) *
          111320 *
          Math.cos(midLat * Math.PI / 180);

        if (
          i > start + 1 &&
          (widthM > maxWidthM || heightM > maxHeightM)
        ) {
          break;
        }
      ...
      const coreFrom = Number(distances[start]) || 0;
      const coreTo = Number(distances[end]) || coreFrom;
      const totalKm = Number(routeIndex.totalKm) || Number(distances[last]) || 0;

      segments.push({
        from: Math.max(0, coreFrom - (segments.length ? overlapKm : 0)),
        to: Math.min(totalKm, coreTo + (end < last ? overlapKm : 0)),
        coreFrom,
        coreTo,
      });
    ...
    if (!segments.length) {
      segments.push({
        from: 0,
        to: Number(routeIndex.totalKm) || 0,
        coreFrom: 0,
        coreTo: Number(routeIndex.totalKm) || 0,
      });
    }
    ...
    if (segments.length >= 2) {
      ...
      const previousKm = previous.coreTo - previous.coreFrom;
      const finalKm = final.coreTo - final.coreFrom;

      if (
        finalKm > 0 &&
        previousKm > 0 &&
        finalKm < previousKm * 0.55
      ) {
        const balancedBoundary =
          (previous.coreFrom + final.coreTo) / 2;

        previous.coreTo = balancedBoundary;
        previous.to = Math.min(
          Number(routeIndex.totalKm) || final.coreTo,
          balancedBoundary + overlapKm
        );

        final.coreFrom = balancedBoundary;
        final.from = Math.max(
          0,
          balancedBoundary - overlapKm
        );
  ...
  function createDetailPrintMaps(
    ...
    const segments = detailSegments(routeIndex);
    ...
    segments.forEach((segment, n) => {
      ...
      if (n % 2 === 0) {
        ...
        sheet.innerHTML = `
          ...
          <footer class="svingom-detail-footer">
            <span>${t.detailFooter}</span>
            <img alt="SvingOm" src="${logoUrl}">
          </footer>`;
        preview.appendChild(sheet);
      } else {
        sheet = preview.lastElementChild;
      }

      mapsEl = sheet.querySelector(".svingom-detail-maps");
      const mapsOnPage = Math.min(2, count - n + (n % 2));
      mapsEl.style.gridTemplateRows =
        `repeat(${mapsOnPage}, minmax(0, 1fr))`;

      const panel = document.createElement("section");
      panel.className = "svingom-detail-panel";
      panel.innerHTML = `
        <div class="svingom-detail-caption">
          <span>${t.detailPart} ${n + 1} / ${count}</span>
      ...
      const fixedScaleZoom = printZoomForScale(center.lat, 50000);
      map.setView(center, fixedScaleZoom, { animate: false });

      addFixedScaleControl(map, t.detailScale);
      addNorthControl(map);
      ...
      if (n === 0) {
        ...
        if (p) L.marker([p.lat, p.lon], {
          icon: createPrintDivIcon("svingom-start-end", t.startMarker, [31, 31], [15, 15]),
          interactive: false,
        }).addTo(map);
      }
      if (n === count - 1) {
        ...
        if (p) L.marker([p.lat, p.lon], {
          icon: createPrintDivIcon("svingom-start-end", t.finishMarker, [31, 31], [15, 15]),
          interactive: false,
        }).addTo(map);
      }
  ...
  function openPrintPreview(
    ...
    if (
      ...
      const loadingTexts = infoTexts[getLang()] || infoTexts.no;
      alert(loadingTexts.printLoading);
    ...
    preview.innerHTML = `
      ...
      <main class="svingom-print-sheet">
        ...
        <footer class="svingom-print-footer">
          ...
          <div class="svingom-print-info-block">
  ...
  function addPrintControl(
    ...
    ensurePrintStyles();
    const printTexts = infoTexts[getLang()] || infoTexts.no;
    ...
    ctrl.onAdd = function () {
      ...
      btn.type = "button";
      btn.title = printTexts.printTitle;

      btn.setAttribute(
        "aria-label",
        printTexts.printTitle
      );
